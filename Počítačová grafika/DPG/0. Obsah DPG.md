[[1. Data structures for searching in multidimensional spaces]]

- [[1. Data structures for searching in multidimensional spaces#Regular and hierarchical data structures, time and space complexity. Cost model and general building and search algorithm for hierarchical data structures.|Regular and hierarchical data structures, time and space complexity. Cost model and general building and search algorithm for hierarchical data structures.]]
- [[1. Data structures for searching in multidimensional spaces#Data structures for representations of points and shapes including nets, special data structures (2.5D, voxel-based representations, point-based data structures).|Data structures for representations of points and shapes including nets, special data structures (2.5D, voxel-based representations, point-based data structures).]]
- [[1. Data structures for searching in multidimensional spaces#Data structures and applications for the nearest neighbor, k-nearest neighbor and range search queries. Data structures and algorithms for high-dimensional search.|Data structures and applications for the nearest neighbor, k-nearest neighbor and range search queries. Data structures and algorithms for high-dimensional search.]]
- [[1. Data structures for searching in multidimensional spaces#Algorithms for building and traversal of the data structures for ray shooting and applications|Algorithms for building and traversal of the data structures for ray shooting and applications.]]
- [[1. Data structures for searching in multidimensional spaces#Data structures for collision detection, the complexity and applications|Data structures for collision detection, the complexity and applications.]]

# Widgety

<iframe 
    src="https://mrshasha.github.io/cvut/Assets/DPG/model-vykonu.html" 
    width="100%" 
    height="640px" 
    frameborder="0" 
    style="border-radius: 8px; border: 1px solid #444;">
</iframe>
<iframe 
    src="https://mrshasha.github.io/cvut/Assets/DPG/kd-vs-bvh.html" 
    width="100%" 
    height="640px" 
    frameborder="0" 
    style="border-radius: 8px; border: 1px solid #444;">
</iframe>
<iframe 
    src="https://mrshasha.github.io/cvut/Assets/DPG/topDown-vs-bottomUp-constr.html" 
    width="100%" 
    height="640px" 
    frameborder="0" 
    style="border-radius: 8px; border: 1px solid #444;">
</iframe>
<iframe 
    src="https://mrshasha.github.io/cvut/Assets/DPG/bbox.html" 
    width="100%" 
    height="640px" 
    frameborder="0" 
    style="border-radius: 8px; border: 1px solid #444;">
</iframe>
<iframe 
    src="https://mrshasha.github.io/cvut/Assets/DPG/bv-intersection.html" 
    width="100%" 
    height="640px" 
    frameborder="0" 
    style="border-radius: 8px; border: 1px solid #444;">
</iframe>
<iframe 
    src="https://mrshasha.github.io/cvut/Assets/DPG/representation.html" 
    width="100%" 
    height="640px" 
    frameborder="0" 
    style="border-radius: 8px; border: 1px solid #444;">
</iframe>
<iframe 
    src="https://mrshasha.github.io/cvut/Assets/DPG/minkowski.html" 
    width="100%" 
    height="640px" 
    frameborder="0" 
    style="border-radius: 8px; border: 1px solid #444;">
</iframe>

<iframe 
    src="https://mrshasha.github.io/cvut/Assets/DPG/nn.html" 
    width="100%" 
    height="640px" 
    frameborder="0" 
    style="border-radius: 8px; border: 1px solid #444;">
</iframe>
<iframe 
    src="https://mrshasha.github.io/cvut/Assets/DPG/kd-nn.html" 
    width="100%" 
    height="640px" 
    frameborder="0" 
    style="border-radius: 8px; border: 1px solid #444;">
</iframe>
<iframe 
    src="https://mrshasha.github.io/cvut/Assets/DPG/ray-shooting.html" 
    width="100%" 
    height="640px" 
    frameborder="0" 
    style="border-radius: 8px; border: 1px solid #444;">
</iframe>
<iframe 
    src="https://mrshasha.github.io/cvut/Assets/DPG/cd.html" 
    width="100%" 
    height="640px" 
    frameborder="0" 
    style="border-radius: 8px; border: 1px solid #444;">
</iframe>

---

[[Všechny widgety]]
