...
**Datově vs. Dokumentově orientované XML:**

- **Datově orientované XML (Data-Centric):** Má pravidelnou a často striktně danou strukturu. Elementy reprezentují hlavně datové položky a pořadí sourozeneckých elementů bývá často méně důležité než jejich název a hodnota (např. zda je první `autor` nebo `nazev`). Hodí se pro strojové zpracování, import/export produktů, konfigurace nebo B2B komunikaci.
- **Dokumentově orientované XML (Document-Centric):** Struktura je volnější a často obsahuje _mixed content_, tedy text prokládaný vnořenými elementy. Typicky jde o lidsky čitelné dokumenty, články, knihy nebo dokumentaci (např. DocBook), kde je důležitý tok a značkování textu, ne primárně agregace dat.
